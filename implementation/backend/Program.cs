...
builder.Services.AddHostedService<ApprovalDecisionQueueProcessorHostedService>();
...
app.MapPost("/requests/{id}/approve", async (string id, ApproveRequestDto dto, HttpContext context, IDataService dataService, IStepFunctionsService stepFunctionsService, IInputSanitizer inputSanitizer, ILogger<Program> logger) =>
{
    ...
    if (string.IsNullOrWhiteSpace(request.ApprovalTaskToken))
    {
        return Results.Problem(
            ...
            detail: "No workflow approval gate token has been stored for this request yet. Please retry shortly.",
    ...
    await stepFunctionsService.SendTaskSuccessAsync(request.ApprovalTaskToken, new Dictionary<string, object>
    {
