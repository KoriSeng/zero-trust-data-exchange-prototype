...
- `POST /requests` - Submit new data access request (Requester)
  - **Request**: `{ "datasetId": "string", "purpose": "string" }`
  - **Security**: Dataset metadata (name, objectKeys, dataOwnerOrg) fetched from catalog database (single source of truth)
  - **Validation**: Verifies dataset exists, is published, and has object keys configured
  - Returns request ID and workflow execution ARN
...
- `POST /requests/{id}/approve` - Approve request and advance workflow to OTP generation (DataOwner/Admin). Supports `accessDurationSeconds` (preferred) and legacy `accessDurationHours`.
- `GET /requests/{id}/otp/email` - View debug OTP email preview generated by workflow (Requester, prototype mode)
- `POST /requests/{id}/download` - Generate short-lived (60s) pre-signed download URL for a specific file during active Redeemed window (Requester). Returns `200` JSON payload containing `downloadUrl`, `objectKey`, and `expiresAt` (no HTTP redirect).
- `POST /requests/{id}/deny` - Deny request (DataOwner/Admin)
- `POST /requests/{id}/redeem` - Redeem request using OTP and get access URLs (Requester)
